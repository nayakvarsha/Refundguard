...
export default function UnifiedLoginModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  // Handle Customer Self-Serve Signup (Step 3 Customer Only!)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5 text-xs text-slate-700">
          ...
          {/* Role Toggle Pills (Step 3: Only toggle role when in LOGIN mode) */}
          {viewMode === 'LOGIN' && (
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  required
                  placeholder={role === 'ADMIN' ? 'Any admin username (e.g. admin, alex, john)' : 'flipkart or finance@flipkart.com'}
                  value={username}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  {/* Step 3 Rule: Show "Forgot password?" ONLY when Customer role is selected! */}
                  {role === 'COMPANY' && (
                    <button
                      type="button"
                      onClick={() => {
              ...
              {/* Step 3 Rule: Show "Sign up" link ONLY when Customer role is selected! */}
              {role === 'COMPANY' && (
          ...
          {/* VIEW 2: CUSTOMER SIGNUP FORM (Customer Only!) */}
          {viewMode === 'SIGNUP' && (
            <form onSubmit={handleSignupSubmit} className="space-y-3.5">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={username}
                  onChange={(e) => setUsername(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 font-mono text-slate-900 font-bold"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-800">Chosen Password</label>
                <input
                  type="password"
                  required
                  placeholder="••••••••••••"
          ...
          {/* VIEW 3: FORGOT PASSWORD REQUEST FORM (Customer Only!) */}
          {viewMode === 'FORGOT' && (
            <form onSubmit={handleForgotSubmit} className="space-y-4">
              <p className="text-slate-600 font-medium">
                Enter your registered customer username or email address to generate a password reset code:
              </p>
          ...
          {/* VIEW 4: RESET PASSWORD COMPLETION FORM */}
          {viewMode === 'RESET' && (
            <form onSubmit={handleResetSubmit} className="space-y-4">
              {generatedCode && (
                <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 space-y-1 text-center font-mono">
                  <span className="text-[10px] text-amber-700 font-bold uppercase tracking-wider block">Demo Verification Reset Code</span>
                  <div className="text-lg font-black text-amber-900 select-all tracking-wider">{generatedCode}</div>
                </div>
              )}

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-800">Verification Reset Code</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-800">New Chosen Password</label>
                <input
                  type="password"
                  required
